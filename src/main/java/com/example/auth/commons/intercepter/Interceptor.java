...
public class Interceptor extends HandlerInterceptorAdapter implements HandlerInterceptor {
    ...
    private final Response response1;
//    @Value("${trs.defaults.timezone}")
    String timezoneValue;

    public Interceptor(RestAPIService restAPIService, JwtTokenUtil jwtTokenUtil, RequestSession requestSession, Response response1) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (restAPIService.hasAccess(Collections.singletonList(Role.ANONYMOUS.toString()), method.getMethod().getName())) {
            try {
                if (jwtToken != null) {
                    JWTUser user = JwtTokenUtil.getJwtUserFromToken(jwtToken);
                    if (user != null) {
        ...
        JWTUser user;
        try {
            user = JwtTokenUtil.getJwtUserFromToken(jwtToken);
            log.info("user" + user);
    ...
    private boolean checkTokenIsExpired(String jwtToken, HttpServletResponse response) throws IOException {
        if (!StringUtils.isEmpty(jwtToken)) {
            boolean expired;
            try {
                expired = jwtTokenUtil.isTokenExpired(jwtToken);
            } catch (Exception e) {
                expired = true;
            }
            if (expired) {
